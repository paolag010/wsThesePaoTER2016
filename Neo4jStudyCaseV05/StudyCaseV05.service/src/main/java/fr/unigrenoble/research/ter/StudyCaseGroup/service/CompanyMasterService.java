...
public class CompanyMasterService {
    ...
    @GET
    @Path("{id}")
    public CompanyDTO getCompanies(@PathParam("id") Long id){
        return companyLogic.getCompanyMaster(id);
    }
    
    @PUT
    @Path("{id}")
    public CompanyDTO updateCompanies(@PathParam("id") Long id, CompanyDTO dto){
        dto.setId(id);
        return companyLogic.updateCompanyMaster(dto);
    }
